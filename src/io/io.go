...
// SeekStart 表示相对于文件的起始处，
// SeekCurrent 表示相对于当前的偏移，而
// SeekEnd 表示相对于其结尾处。
// Seek 返回相对于文件起始处新的偏移量和一个错误，如果有的话。
//
// 对文件之前的偏移量进行 Seek 会产生错误。
// 对任何正数偏移量进行 Seek 是合法的，但对底层类型的后续
// I/O 操作行为则取决于具体实现。
...
// 若 w 实现了 WriteString 方法，就可以直接调用它。否则，w.Write 只会被调用一次。
